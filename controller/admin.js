...
const ProductSubCategory = require('../models/sub-category');
...
exports.getLoginAdminInfo = async (req, res, next) => {
    ...
    try {
        ...
        const result = await Admin.findOne({_id: loginUserId})
            .select('-password')
...
exports.checkAdminByPhone = async (req, res, next) => {
    ...
    try {
        ...
        if (await Admin.findOne({phoneNo: phoneNo})) {
            res.status(200).json({
                data: true,
...
exports.editAdminPassword = async (req, res, next) => {
    ...
    try {
        ...
        await Admin.findOneAndUpdate({phoneNo: phoneNo}, {$set: {password: hashedPass}});
